...
def add_step_ovr(m):
    bot.send_message(m.chat.id, f"2️⃣ Введите общий рейтинг OVR для {m.text}:")
    bot.register_next_step_handler(m, add_step_event, m.text)

def add_step_event(m, name):
    bot.send_message(m.chat.id, f"3️⃣ Введите НАЗВАНИЕ СОБЫТИЯ:")
    bot.register_next_step_handler(m, add_step_pos, name, m.text)

def add_step_pos(m, name, ovr):
    bot.send_message(m.chat.id, f"4️⃣ Введите ОСНОВНУЮ ПОЗИЦИЮ (строго одну из: LF, CF, RF, CM, LB, RB):")
    bot.register_next_step_handler(m, add_step_rarity, name, ovr, m.text)
